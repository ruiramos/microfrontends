...
const DisableTreeShakingForChunkPlugin = require("disable-tree-shaking-for-chunk-plugin");

class DisableTreeShaking {
  PluginName = "DisableTreeShaking";
    ...
    compiler.hooks.thisCompilation.tap(
      ...
      (compilation) => {
        ...
        compilation.hooks.seal.tap("FlagEntryExportAsUsedPlugin", () => {
          if (!compilation.entryDependencies) return;
  ...
  oldapply(compiler) {
    compiler.hooks.compilation.tap(this.PluginName, (compilation) => {
      compilation.hooks.afterOptimizeChunkModules.tap(
        this.PluginName,
        (chunks) => {
          const targetChunks = chunks.filter((chunk) => true);
          ...
          targetChunks.forEach((targetChunk) => {
            targetChunk.modulesIterable.forEach((m) => {
              if (m.type.startsWith("javascript/")) {
                if (m.context && m.context.indexOf("react-router") > -1) {
                  console.log(m);
                  m.usedExports = m.buildMeta.providedExports; //true;
                  m.used = true;
                }
